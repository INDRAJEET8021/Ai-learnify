...
export default function QuizComponent() {
  ...
  const handleSkip = () => {
    setSkippedQuestions([...skippedQuestions, questions[currentQuestionIndex].id]);
    setCurrentQuestionIndex((prev) => prev + 1);
  };

  const handleMarkForReview = () => {
    setMarkedForReview([...markedForReview, questions[currentQuestionIndex].id]);
    setCurrentQuestionIndex((prev) => prev + 1);
  };
    ...
    <Box sx={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      {questions.length > 0 && !showReport && (
        <Box>
          ...
          <Paper sx={{ padding: '20px', borderRadius: '12px', marginBottom: '20px' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {questions[currentQuestionIndex].options.map((option, index) => (
                <Button
                  key={index}
                  variant={answers[questions[currentQuestionIndex].id] === option ? 'contained' : 'outlined'}
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <IconButton onClick={goToPreviousQuestion} disabled={currentQuestionIndex === 0}>
              <ArrowBack />
            </IconButton>
            <Box>
              <Button variant="outlined" onClick={handleSkip} sx={{ marginRight: '10px' }}>
                Skip
              </Button>
              <Button variant="outlined" onClick={handleMarkForReview} sx={{ marginRight: '10px' }}>
                Mark for Review
              </Button>
            </Box>
            <IconButton onClick={goToNextQuestion} disabled={currentQuestionIndex === questions.length - 1}>
              <ArrowForward />
            </IconButton>
          </Box>
          ...
          {currentQuestionIndex === questions.length - 1 && (
            <Button variant="contained" color="success" fullWidth onClick={handleFinishQuiz}>
              Finish Quiz
            </Button>
          )}
        </Box>
